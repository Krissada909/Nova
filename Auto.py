...
# ตั้งค่า logging
logging.basicConfig(filename="auto_reply_log.txt", level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
...
def auto_reply_outlook(folder_name):
    try:
        ...
        # CSV file setup
        file_name = "replied_emails.csv"
        file_exists = os.path.exists(file_name)
        with open(file_name, mode="a", newline="", encoding="utf-8-sig") as csv_file:
            ...
            if not file_exists:
                writer.writerow(["Sender", "Subject", "Urgency", "Ticket Type", "Reply Time"])
            ...
            for message in messages:
                if message.Unread:
                    ...
                    if found_keyword:
                        ...
                        "หย่อน UOM ผิดช่อง","หย่อนรหัสสินค้าผิดช่อง"]
                        for part in parts[1:]:
                    ...
                    # Save reply information to CSV
                    writer.writerow([message.SenderName, message.Subject, urgency, ticket_type, message.ReceivedTime])
                    print(f"Saved to CSV: {message.Subject}")
